...
pub trait HasRelative<'a>: Sized {
    fn relative<K>(&self, path: impl IntoIterator<Item = K>) -> Option<Self>
    where K: Into<String>,
        Self: Clone + HasRoot + HasParent + HasBranches<Self> + HasPathSegment + 'a,
        &'a Self: HasBranches<Self> + HasPathSegment, 
    {
        #[inline]
        ...
        if let Some(segment) = path.next() {
            ...
            let visitor: Self = match segment.kind() {
                ...
                PathSegment::Other(_) => longer_ref(self).get::<Self>(segment)?
            };
