use ::is_tree::*;
use super::super::*;

impl<'a> KnowsVisitorOf<'a, String> for &'a Module {
    type Visitor = Visitor<Visitors<'a, &'a Library, &'a Module>, &'a String>;
}

impl<'a> KnowsVisitorOf<'a, String> for &'a mut Module {
    type Visitor = Visitor<Visitors<'a, &'a Library, &'a Module>, &'a mut String>;
}

impl<'a> TypeIter<'a, Visitor<Visitors<'a, &'a Library, &'a Module>, &'a String>> for &'a Module {
    fn type_iterator(self, parent: Visitors<'a, &'a Library, &'a Module>) -> TypeIterator<Visitor<Visitors<'a, &'a Library, &'a Module>, &'a String>> {
        let mut collection = Vec::new();
        collection.push(Visitor::new(parent.clone(), &self.name));
        collection.extend(self.children.iter().flat_map(|child| child.iter_type_with_parent::<String>(Visitor::new(parent.clone().into(), self).into())));
        collection.into()
    }
}

impl<'a> TypeIter<'a, Visitor<Visitors<'a, &'a Library, &'a Module>, &'a mut String>> for &'a mut Module {
    fn type_iterator(self, parent: Visitors<'a, &'a Library, &'a Module>) -> TypeIterator<Visitor<Visitors<'a, &'a Library, &'a Module>, &'a mut String>> {
        let mut collection = Vec::new();
        let self_as_parent: Visitor<Visitors<'a, &'a Library, &'a Module>, &'a Module> = Visitor::new(parent.clone().into(), unsafe { &mut *(self as *mut Module) }).into();
        collection.push(Visitor::new(parent.clone(), &mut self.name));
        collection.extend(self.children.iter_mut().flat_map(|child| child.iter_type_with_parent::<String>(self_as_parent.clone().into())));
        collection.into()
    }
}

