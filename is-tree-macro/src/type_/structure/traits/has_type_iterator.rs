...
pub fn impl_has_type_iterator(structure: &Structure) -> TokenStream {
    ...
    if let (Some(type_), Some(visitor)) = (has_type_iterator, knows_visitor) {
        ...
        let collect: TokenStream = structure.fields.iter().map(|field| {
            let type_name = type_.get_ident().unwrap().to_string();
            let field_name = &field.field.ident;
            if let Some(type_) = field.as_collection() {


                // FIXME: It seems like it isn't working.
                if field.is_any_type_of(&[type_.get_ident().unwrap().to_string().as_str()]) {
                    quote! {
                        collection.extend(self.#field_name.iter().flat_map(|child| ::is_tree::Visitor::new(self.visitor().into(), child)));
                    }
                } else {
                    quote! {
                        collection.extend(self.#field_name.iter().flat_map(|child| child.iter_type_with_parent::<#type_>(Some(self.visitor().into()))));
                    }
                }
                ...
                if field.is_any_type_of(&[type_name.as_str()]) {
                    quote! {
                        collection.push(::is_tree::Visitor::new(self.visitor().into(), &self.#field_name));
                    }
                } else {
                    quote! {
                        collection.extend((&self.#field_name).iter_type_with_parent::<#type_>(Some(self.visitor().into())));
                    }
                }
            }
        }).collect();
